...
export const IntentEntity = () => {
  ...
  const totalItems = useAppSelector(state => state.intentEntity.totalItems);
  ...
  return (
    <div>
            ...
            <thead>
              <tr>
                ...
                <th className="hand" onClick={sort('optional')}>
                  Optional <FontAwesomeIcon icon={getSortIconByFieldName('optional')} />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {intentEntityList.map((intentEntity, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{intentEntity.name}</td>
                  <td>{intentEntity.optional ? 'true' : 'false'}</td>
                  <td className="text-end">
                    <div className="btn-group flex-btn-group-container">
                      ...
                      <Button
                        tag={Link}
                        to={`/intent-entity/${intentEntity.id}/edit?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="primary"
                        size="sm"
                      ...
                      <Button
                        onClick={() =>
                          (window.location.href = `/intent-entity/${intentEntity.id}/delete?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`)
                        }
                        color="danger"
                        size="sm"
      ...
      {totalItems ? (
        <div className={intentEntityList && intentEntityList.length > 0 ? '' : 'd-none'}>
